...
static int mcu_bload_update_fw(struct i2c_client *client)
{
	...
	wbuf = (unsigned char*)kmalloc(MAX_BUF_LEN, GFP_KERNEL);
			     ...
			     int mode, int frate_index)
{
...
#ifdef AR1335_DEBUG
	dev_info(&client->dev," Index = 0x%04x , format = 0x%08x, width = %hu,"
	...
	if(prev_index == index) {
#ifdef AR1335_DEBUG
		dev_info(&client->dev, "Skipping Previous mode set ... \n");
#endif
		ret = 0;
		goto exit;
	}
...
static int ar1335_enum_frameintervals(struct v4l2_subdev *sd,struct v4l2_subdev_state *sd_state,struct v4l2_subdev_frame_interval_enum *fival)
{
...
static int ar1335_enum_framesizes(struct v4l2_subdev *sd,struct v4l2_subdev_state *sd_state,struct v4l2_subdev_frame_size_enum *fse)
{
...
static int ar1335_enum_mbus_code(struct v4l2_subdev *sd,struct v4l2_subdev_state *sd_state,struct v4l2_subdev_mbus_code_enum *code)
{
...
static int ar1335_get_fmt(struct v4l2_subdev *sd,struct v4l2_subdev_state *sd_state,struct v4l2_subdev_format *format)
{
	int ret = 0;
...
static int ar1335_set_fmt(struct v4l2_subdev *sd, struct v4l2_subdev_state *sd_state,
...
static int ar1335_verify_mcu(struct i2c_client *client)
{
	...
	if (gpios_available_1335())
	{
		...
		if (ret == 0)
		{
			//dev_info(&client->dev,"%s: Skipping verify fw version for now. Use CAM137 f/w",__FUNCTION__);
			...
			const struct i2c_device_id *id)
{
	...
	ret = of_property_read_u32(node, "camera_mipi_lanes", (u32 *)&(ar1335_data.mipi_lane_config));	
	if (ret) {
		dev_err(&client->dev, "camera mipi lane is missing or invalid\n");
		return ret;
	}
	printk("Camera MIPI Lanes =%d\n", ar1335_data.mipi_lane_config);
	...
	dev_info(dev, "SUBDEV: name=%s", ar1335_data.subdev.name);
	dev_info(dev, "SUBDEV: addr=%X", &ar1335_data.subdev);

	dev_info(dev, "AR1335 detected.");
...
MODULE_DESCRIPTION("AR1335_old V4L2 driver 2");
